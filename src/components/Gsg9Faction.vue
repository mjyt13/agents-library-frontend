...
const currentId = ref(gsg.subfractions[0]?.id ?? '')

const currentSubfraction = computed(() => gsg.subfractions.find((sub) => sub.id === currentId.value) ?? null)
</script>

<template>
  <div class="gsg9FactionPage">
    <div class="switcher">
      <button
        v-for="sub in gsg.subfractions"
        :key="sub.id"
    ...
    <SubfractionPage v-if="currentSubfraction" :subfraction="currentSubfraction" :faction-name="gsg.name" />
  </div>
</template>
