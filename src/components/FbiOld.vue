...
const currentId = ref(fbi_old.subfractions[0]?.id ?? '')

const currentSubfraction = computed(
  () => fbi_old.subfractions.find((sub) => sub.id === currentId.value) ?? null,
)
</script>

<template>
  <div class="fbiOldPage">
    <div class="switcher">
      <button
        v-for="sub in fbi_old.subfractions"
        :key="sub.id"
