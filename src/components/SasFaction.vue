...
const currentId = ref(sas.subfractions[0]?.id ?? '')

const currentSubfraction = computed(() => sas.subfractions.find((sub) => sub.id === currentId.value) ?? null)
</script>

<template>
  <div class="sasFactionPage">
    <div class="switcher">
      <button
        v-for="sub in sas.subfractions"
        :key="sub.id"
    ...
    <SubfractionPage v-if="currentSubfraction" :subfraction="currentSubfraction" :faction-name="sas.name" />
  </div>
</template>
