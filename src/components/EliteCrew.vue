...
const currentId = ref(leet.subfractions[0]?.id ?? '')

const currentSubfraction = computed(() => leet.subfractions.find((sub) => sub.id === currentId.value) ?? null)
</script>

<template>
  <div class="eliteCrewPage">
    <div class="switcher">
      <button
        v-for="sub in leet.subfractions"
        :key="sub.id"
    ...
    <SubfractionPage v-if="currentSubfraction" :subfraction="currentSubfraction" :faction-name="leet.name" />
  </div>
</template>
